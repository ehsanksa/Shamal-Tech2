...
import { shamalSeed } from './shamal-seed'
...
export const seed = async ({
  ...
  // clear the database - only clear Header and Footer globals (they have navItems)
  // Other globals will be handled by shamal-seed function
  await Promise.all(
    globals.map((global) =>
      payload.updateGlobal({
        ...
        data: {
          navItems: [],
        } as any, // Type assertion needed as navItems may not be in all global types
        depth: 0,
        context: {
  ...
  // Always run Shamal Technologies specific seeding
